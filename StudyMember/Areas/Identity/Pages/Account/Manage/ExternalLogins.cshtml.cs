...
namespace StudyMember.Areas.Identity.Pages.Account.Manage
{
    public class ExternalLoginsModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetLinkLoginCallbackAsync()
        {
            ...
            if (info == null)
            {
                throw new InvalidOperationException($"Um erro inesperado ocorreu ao carregar informações de login externo.");
            }

            var result = await _userManager.AddLoginAsync(user, info);
            if (!result.Succeeded)
            {
                StatusMessage = "O login externo não foi adicionado. Login Externos podem ser associados somente com uma conta";
                return RedirectToPage();
            }
